...
void FAnimationViewportClient::CalcBoneSpeeds(TArray<float>& OutSpeeds, const FName& BoneName)
{
	...
	FBoneContainer& BoneContain = PreviewInstance->GetRequiredBones();

	FCompactPose LocalPose;
	LocalPose.SetBoneContainer(&BoneContain);

	FBlendedCurve EmptyCurve;

	int32 BoneIndex = BoneContain.GetReferenceSkeleton().FindBoneIndex(BoneName);
	if (BoneIndex < 0)
		return;

	for (int32 Frame = 0; Frame < AnimSequence->NumFrames; ++Frame)
	{
		float Time = AnimSequence->GetTimeAtFrame(Frame);

		AnimSequence->GetBonePose(LocalPose, EmptyCurve, FAnimExtractContext(Time));

		FCSPose<FCompactPose> WorldPose;
		WorldPose.InitPose(LocalPose);

		FCompactPoseBoneIndex PoseIndex = BoneContain.GetCompactPoseIndexFromSkeletonIndex(BoneIndex);
		FVector BonePos = WorldPose.GetComponentSpaceTransform(PoseIndex).GetTranslation();
	...
	for (int32 PosIndex = 1; PosIndex < BonePosList.Num(); ++PosIndex)
	{
		float BoneSpeed = (BonePosList[PosIndex] - BonePosList[PosIndex - 1]).Size();

		OutSpeeds.Add(BoneSpeed);
	}
}
